...
class admin_right_controller extends adminCommon{
    ...
    function ajax_statis_action(){
        ...
        $mchatwhere['sendTime']	=	array('>=',$month);
        $chatNumMon			=	$ChatM->getChatLogNum($mchatwhere);

        $mujobwhere['datetime'] = array('>=',$month);
        $userjobNumMon = $JobM->getSqJobNum($mujobwhere);

        $myqmswhere['datetime'] = array('>=',$month);
        $yqmsNumMon = $JobM->getYqmsNum($myqmswhere);

        $mdownresumewhere['downtime'] = array('>=',$month);
    ...
    function ujobtj_action(){
        $this->tj("userid_job", array('datetime'), array('简历投递统计'));
    }
    function yqmstj_action(){
        $this->tj("userid_msg", array('datetime'), array('邀请面试统计'));
    }
    function downresumetj_action(){
        $this->tj("down_resume", array('downtime'), array('简历下载统计'));
    }
    function resumetj_action(){
        $this->tj("resume_expect", array('ctime','r_time'), array('简历新增','简历刷新'));
    }
    ...
    function jobtj_action(){
        $this->tj("company_job", array('sdate','r_time'), array('职位新增','职位刷新'));
    }
    ...
    function chattj_action(){
        $this->tj("chat_log", array('sendTime'), array('聊天统计'));
    }
    ...
    function dt($tablename, $field, $name, $where=array()){
        ...
        if (is_array($List)) {
            ...
            foreach ($List as $k => $v) {
                ...
                if (!empty($memberLt)){
                    foreach ($memberLt as $val) {
                        
                        if ($v['comid'] == $val['uid']) {
                            
                            $List[$k]['comusername'] =	$val['realname'];
                        }
                    }
                }
