		...
		if($_POST){
 			...
 			$clearData=@implode(',',$_POST['clear_data']);
  			...
  			if($clearData==""){
                ...
                if (in_array('chat_log', $_POST['clear_data'])) {//  聊天记录
                    $sendTime   =   $clearTime * 1000;
                    $where['sendTime'] = array('<', $sendTime);
                    $nid = $this->MODEL('chat')->delChatLog(array('where' => $where, 'norecycle' => 1));
                }
                if (in_array('web_log', $_POST['clear_data'])){//  行为轨迹
                    $where['time'] = array('<', $clearTime);
                    $nid = $this->MODEL('weblog')->delWeblog($where, array('norecycle' => 1));
                }
