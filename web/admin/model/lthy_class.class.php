...
class lthy_class_controller extends adminCommon{
	function index_action(){
		...
		$whereData['keyid'] =	'0';
		$position			=	$categoryM -> getLthyClassList(array('keyid'=>'0'));
	...
	function save_action(){
		...
		$categoryM 				= $this   -> MODEL('category');
		$return					= $categoryM -> addLthyClass($addData);
		echo $return['msg'];die;
	}
	//分类管理
	function up_action(){
		...
		if($_GET['id']){
			...
			$onejob				= 	$categoryM	->	getLthyClass($whereOne);
			$twojob				=	$categoryM	->	getLthyClassList($whereTwo);
			$this	->	yunset("id",$id);
	...
	function del_action(){
		...
		$return	=	$categoryM	->	delLthyClass($whereData,$data);
	...
	function ajax_action(){
		...
		$categoryM	->	addLthyClass($addData,$whereData);
		echo '1';
		die;
	}
}
