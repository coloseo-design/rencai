...
class ltclass_controller extends adminCommon{
	function index_action(){
		...
		$whereData['keyid'] =	'0';
		$position			=	$categoryM -> getLtClassList(array('keyid'=>'0'));
	...
	function save_action(){
		...
		$categoryM 				= $this   -> MODEL('category');
		$return					= $categoryM -> addLtClass($addData);
		echo $return['msg'];die;
	}
	//分类管理
	function up_action(){
		...
		if($_GET['id']){
			...
			$class1				= 	$categoryM	->	getLtClass($whereOne);
			$class2				=	$categoryM	->	getLtClassList($whereTwo);
			$this	->	yunset("id",$id);
	...
	function del_action(){
		...
		$return	=	$categoryM	->	delLtClass($whereData,$data);
	...
	function ajax_action(){
		...
		$categoryM	->	addLtClass($addData,$whereData);
		echo '1';
		die;
	}
}
