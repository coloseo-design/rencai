...
class com_pl_controller extends adminCommon{
	function set_search(){
		$search_list[]=array("param"=>"status","name"=>'审核状态',"value"=>array('0'=>'未审核','1'=>'已审核','2'=>'已拒绝'));
	...
	function lockinfo_action(){
	    ...
	    $msg     =  $companyM -> getCompanyMsgInfo(array('id'=>$_POST['cid']),array('field'=>'statusbody'));
	    
	    echo trim($msg['statusbody']);die;
	}
	function status_action(){
        ...
        $return     =   $companyM -> statusComment(array('id' => array('in', pylode(',', $cids))), array('post' => $post));
		
		if($return['errcode']==9){
			
			$this  ->  ACT_layer_msg($return['msg'],$return['errcode'],"index.php?m=com_pl",2,1);
	...
	function show_action(){
		...
		$info        =    $companyM->getCompanyMsgInfo(array('id' => $id), array('field'=>'content,othercontent,reply','type'=>'admin')); 
