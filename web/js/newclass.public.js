...
function index_industry_new(allow_select_industry_count, target_industry_names, target_industry_ids,industrydiv_style,codeids, index_industry_callback) {
	...
	$(hi).each(function (i, data) {
		listHtml.push("<li><div class=\"category categorylt\"><label><input "+window.style+" type=\"checkbox\" value=\""+data+"\" name=\""+hyname[data]+"\" />"+hyname[data]+"</label></div><div class=\"detail\" ><ul>");
		$(ht[data]).each(function(i,datas){
			listHtml.push("<li><div class=\"\"><div class=\"flo_lin flo_lin_lt\"><label><input value=\""+datas+"\" hyonevalue=\""+data+"\" name=\""+hyname[datas]+"\"  type=\"checkbox\">"+hyname[datas]+"</label></div></div></li>");
		});	
		listHtml.push("</ul></div><div class=\"clear\"></div></li>");
	});
	$(".a_prof_n").html(listHtml.join(""));
		...
		$(hi).each(function (i, data) {
			for(var i=0;i<IndustryArrayAll.length;i++){
				if(IndustryArrayAll[i]==data){
					$("#ChooseIndustryLayer .ShowIndustry").append("<div class=\"job\" code=\"" +data+ "\" type=\"hyone\" title=\"" +hyname[data]+ "\" ><span>" + hyname[data] + "</span><label class=\"job_lab \"></label></div>");
			...
			$(ht[data]).each(function(i,data2){
				for(var i=0;i<IndustryArrayAll.length;i++){
					if(IndustryArrayAll[i]==data2){
						$("#ChooseIndustryLayer .ShowIndustry").append("<div class=\"job\" hyonecode=\""+data+"\" code=\"" + data2 + "\" type=\"hytwo\" title=\"" +hyname[data2]+ "\" ><span>" +hyname[data2] + "</span><label class=\"job_lab \"></label></div>");
