...
class public_controller extends wxapp_controller{
	...
	function com_action(){//获取企业类别
		...
		if ($_POST['uid']){
			...
			if($_POST['sid'] && $_POST['sid']>0){
		    	...
		    	$row	=	$this -> MODEL('job') -> getInfo(array('id'=>$_POST['jobid']),array('field'=>'`number`,`report`,`age`,`lang`,`edu`,`exp`,`marriage`,`sex`,`exp_req`,`edu_req`'));
	...
	function keyword_action(){  //关键字查询
		...
		$rows = $hotKeyM -> getList($where,array('field'=>'`key_name`,`type`'));
		foreach($rows as $k=>$v) {
		    $key_name[]	=	$v['key_name'];
		}
