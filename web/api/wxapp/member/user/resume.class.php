...
class resume_controller extends user_controller{
	...
	function saveresumeson_action()
	{
		...
		if($_POST['eid']>0){
			...
			if(!empty($reusmeData) && $nid){

                $pass = $ResumeM->resumeConCheck($reusmeData,array('uid'=>$this->member['uid'],'usertype'=>1,'eid'=>(int)$_POST['eid'],'source'=>$_POST['source']));

                if(!$pass){
					$ResumeM->upInfo(array('id'=>(int)$_POST['eid'], 'uid'=>$this->member['uid']), array('eData'=>array('state'=>0)));
                }
	...
	function look_resume_del_action()//删除简历浏览记录
	{
        ...
        $return       	=  $lookResumeM -> delInfo(array('id'=>$del,'uid'=>$this->member['uid'],'usertype'=>1));
        $data['error']	=  $return['errcode']==9 ? 1 : 2;
		$this -> render_json($data['error'],$return['msg'],'');
	}
	//检查简历创建基本信息
	...
	function showSave_action(){
		...
		if(!empty($reusmeData) && $nid){
			...
			if(!$pass){
				$ResumeM->upResumeTable('resume_show',array('id'=>$nid,'uid'=>$this->member['uid']),array('status'=>1, 'eid' => $_POST['eid']),array('utype'=>'user'));
			}
		}
		$nid?$data['msg']	=	'保存成功！':$data['msg']='保存失败！';
		$nid?$data['error']	=	'1' : $data['error'] = '2';
	...
	function resumeset_action(){
        ...
        if($this->config['sy_lietou_web'] == 1){
            ...
            if(!empty($rows)&&$id!=$rows['id']){
                ...
                if($gdeu!=1){
                    $this->render_json(0,'学历本科以上才可以申请优质简历！');
                }
                $workwhere['eid']=	$id;
                $workwhere['uid'] =	$uid;
                ...
                if(!($whour>24 || $worknum>3)){
                    if ($row['whour']<24){
                        $this->render_json(0,'工作经历二年以上才可以申请优质简历！');
                    }elseif ($worknum<4){
                        $this->render_json(0,'工作经历三项以上才可以申请优质简历！');
                    }
                }
                if($this->config['user_height_resume']=='2'){
                    $resumeexpectData = array('height_status'=>	2,'status_time'=>time());
                    $resumeexpectwhere['id'] = $id;
                    $resumeexpectwhere['uid'] =	$uid;
                    $nid =	$resumeM->upInfo($resumeexpectwhere,array('eData'=>$resumeexpectData));
                    $msg="申请成功！";
                }else{
                    $resumeexpectData = array('height_status'=>1,'status_time'=>time());
                    $resumeexpectwhere['id'] = $id;
                    $resumeexpectwhere['uid'] =	$uid;
                    $nid =	$resumeM->upInfo($resumeexpectwhere,array('eData'=>$resumeexpectData));
                    $msg="申请成功，请等待审核！";
                    ...
                    $this->MODEL('admin')->sendAdminMsg(array('first'=>'有新的优质简历需要审核，用户《'.$resume['name'].'》申请优质简历《'.$expect['name'].'》','type'=>10));
                }
                if($nid){
                    $this->render_json(1,$msg);
                }else{
                   $this->render_json(0,'申请失败！');
                }
            }
        }else{
            $this->render_json(0,'系统暂未开启此功能');
        }
    }
