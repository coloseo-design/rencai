...
class msg_controller extends com_controller{
	...
	function sysnews_action(){
	    ...
	    if(!empty($this->config['sy_chat_open']) && $this->config['sy_chat_open'] == 1){
	        ...
	        $unread   =  $chatM->getChatLogNum(array('to'=>$this->member['uid'],'tusertype'=>$this->member['usertype'],'status'=>2,'sendTime'=>array('>',$time),'orderby'=>'id'));
	    }
		$JobM		  =  $this -> MODEL('job');
		// 对我感兴趣（查看职位）
		...
		$list['sysnum']  	 =  $unread + $sxnum + $userid_jobnum + $jobnum;
		$list['ischat']      =  isset($this->config['sy_chat_open']) ? $this->config['sy_chat_open'] : 2;
	...
	function saveplreply_action(){
    	...
    	if(!$_POST['id']){
     		$data['error']    = 3;
      		...
      		$where['id']          =   (int)$_POST['id'];
      		$where['cuid']     =   $this->member['uid'];
      		$nid =   $CompanyM->upReplyCompanymsgInfo($where,$data);
      		if($nid){
				$LogM			=	$this -> MODEL('log');
				$LogM->addMemberLog($this->member['uid'],$this->member['usertype'],"回复面试评价",18,1);
				$data['error']  =   1;
				$data['errmsg']	=	'回复成功';
      		}else{
				$data['error']  =  2;
