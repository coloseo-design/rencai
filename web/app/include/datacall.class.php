...
class datacall{
	...
	public function lt_job($row){
		...
		if(is_array($rows)){
			foreach($rows as $key=>$va){
				if(strstr($row['code'],"{jobname}")){
					$data[$key]['jobname']		=	mb_substr($va['job_name'],0,$row['titlelen'],$this->charset);
				}
				if(strstr($row['code'],"{url}")){
					$data[$key]['url']			=	Url('lietou',array("c"=>"job_show","id"=>$va['id']));
				}
				if(strstr($row['code'],"{companyname}")){
					$data[$key]['companyname']	=	mb_substr($va['com_name'],0,$row['titlelen'],$this->charset);
				}
				...
				if(strstr($row['code'],"{address}")){
					$data[$key]['address']		=	$city_name[$va['provinceid']]."-".$city_name[$va['cityid']];
				}
				if(strstr($row['code'],"{department}")){
					$data[$key]['department']	=	$va['department'];
				}
				if(strstr($row['code'],"{hy}")){
					$data[$key]['hy']			=	$industry_name[$va['hy']];
				}
				...
				if(strstr($row['code'],"{report}")){
					$data[$key]['report']		=	$va['report'];
				}
				if(strstr($row['code'],"{jobtype}")){
					$data[$key]['jobtype']		=	$ltjob_name[$va['jobone']]."-".$ltjob_name[$va['jobtwo']];
				}
				if(strstr($row['code'],"{constitute}")){
					$data[$key]['constitute']	=	$ltclass_name[$va['constitute']];
				}
				if(strstr($row['code'],"{sdate}")){
					$data[$key]['sdate']		=	date("$row[timetype]",$va['lastupdate']);
				}
				//if(strstr($row['code'],"{edate}"))$data[$key]['edate']=date("$row[timetype]",$va['edate']);
				if(strstr($row['code'],"{job_desc}")){
					$data[$key]['desc']			=	mb_substr($va['desc'],0,$row['infolen'],$this->charset);
				}
				if(strstr($row['code'],"{salary}")){
					$data[$key]['salary']		=	$ltclass_name[$va['salary']];
				}
				if(strstr($row['code'],"{edu}")){
					$data[$key]['edu']			=	$ltclass_name[$va['edu']];
				}
				if(strstr($row['code'],"{sex}")){
					$data[$key]['sex']			=	$arr_data['sex'][$va['sex']];
				}
				if(strstr($row['code'],"{language}")){
					$data[$key]['language']		=	$ltclass_name[$va['language']];
				}
				if(strstr($row['code'],"{full}")){
					$data[$key]['full']			=	$ltclass_name[$va['full']];
				}
				if(strstr($row['code'],"{age}")){
					$data[$key]['age']			=	$ltclass_name[$va['age']];
				}

				if(strstr($row['code'],"{exp}")){
					$data[$key]['exp']			=	$ltclass_name[$va['exp']];
				}
