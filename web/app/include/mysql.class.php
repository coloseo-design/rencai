...
class mysql {
	...
	public function query($sql) {
		...
		//$sqlstatic['stime'] = microtime(true);
		...
		$this->sql = $sql;
		//file_put_contents(APP_PATH."file.txt",$this->sql.";\n",FILE_APPEND);
		
		//记录下所有 修改 删除SQL语句 位置不能错 否则会影响正常SQL执行
		//截取SQL语句的前7个字符
		$sqlprx = mb_substr(trim($this->sql),0,7);
		...
		if(stripos($sqlprx,'update')!==false && stripos($this->sql,'company_statis')!==false){
			...
			mysql_query("INSERT INTO `phpyun_z_sqlinfo` SET `sqlinfo` ='".$this->escape_string($this->sql)."',`time`='".date('Y-m-d H:i:s')."',`uri`='".$_SERVER["REQUEST_URI"]."'",$this->conn);
		}
