 ...
 */
 class cache{
	...
	public function lthy_cache($dir){
        ...
        $this->made_web_js($this->cachedir.'ltindustry.cache.js',array('hi'=>$lthy_index,'ht'=>count($lthytype)?$lthytype:'new Array()','hyname'=>$lthyname));
	...
	public function subject_cache($dir){
		$rows	=	$this->obj->select_all("px_subject_class",array('orderby'=>'sort,desc'));
		if(is_array($rows)){
			foreach($rows as $v){
				if($v['keyid']==0){
					$subject_index[]			=	$v['id'];
	...
	public function subject_type_cache($dir){
		$rows	=	$this->obj->select_all("px_subject_type",array('orderby'=>'sort,desc'));
		if(is_array($rows)){
			foreach($rows as $v){
	...
	public function crm_cache($dir){
	    
	    $rows  =   $this->obj->select_all('crm_client_class',array('orderby' => 'sort,asc'));
	    ...
	    $data['crmdata']		=	ArrayToString($data2);
	    $data['crmclass_name']	=	ArrayToString($crmname);
	    return made_web_array($this->cachedir.$dir,$data);
	}
    ...
    public function school_cache($dir){
		$rows	=	$this->obj->select_all("schoolclass",array('orderby'=>'sort,asc'));
		if(is_array($rows)){
			foreach($rows as $v){
				if($v["keyid"]!=0){
