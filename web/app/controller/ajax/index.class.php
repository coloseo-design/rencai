...
class index_controller extends common{	
	...
	function lt_down_resume_action(){
		...
		$arr			=	$downresumeM -> downResume($data);

		if($arr['status']==3){
			$expect		=	$resumeM->getExpect(array('id'=>intval($_POST['eid'])),array('field'=>'`uid`'));

			$user		=	$resumeM->getResumeInfo(array('uid'=>$expect['uid'],'r_status'=>1),array('field'=>'`telphone`,`telhome`,`email`'));
	...
	function DefaultLoginIndex_action(){
		...
		if($this->usertype=='1' && $this->uid){
			...
			$company['msgnum']		=	$MsgM -> getMsgNum(array('job_uid'=>$this->uid,'status'=>1));
			...
			$lt		=	$lietouM->getInfo(array('uid'=>$this->uid),array('field'=>'`realname`,`photo`'));
			
			$lt['lt_job']		=	$ltjobM->getLtjobNum(array('uid'=>$this->uid,'status'=>1));
			$lt['lt_status2']	=	$downM->getDownNum(array('uid'=>$this->uid,'usertype'=>3,'isdel'=>9));
			$lt['entrust']		=	$entrustM->getEntrustNum(array('lt_uid'=>$this->uid));

			$this->yunset("lt",$lt);

		}elseif($this->usertype=='4' && $this->uid){
			$trainM=$this->MODEL('train');
			$member	=	$trainM->getInfo(array('uid'=>$this->uid),array('field'=>'`name`,`logo`'));
			
			$member['subject']	=	$trainM->getPxSubjectNum(array('uid'=>$this->uid));
			$member['baoming']	=	$trainM->getPxBaomingNum(array('s_uid'=>$this->uid,'status'=>0));
			$member['zixun']	=	$trainM->getZixungNum(array('s_uid'=>$this->uid,'status'=>1));
	...
	function pubqrcode_action(){//公共二维码跳转
		...
		if(($this -> config['sy_ewm_type'] == 'weixin' || $this -> config['sy_ewm_type'] == 'xcx') && $_GET['toa'] !='whb' && $_GET['totype'] != 'wxpubtool' && !$_GET['hb']){
			...
			if( isset($_GET['toid']) && $_GET['toid'] != ''){
			    if ($_GET['hb']){
			        $wapUrl = Url('wap',array('c'=>$_GET['toc'],'a'=>$_GET['toa'],'id'=>(int)$_GET['toid'], 'hb' => 1));
                }else{
	...
	function showrebates_action(){
		if(intval($_POST['id'])){
			$lietouM	=	$this -> MODEL('lietou');
			$data		=	$lietouM -> getRebatesInfo(array('id'=>intval($_POST['id'])),array('type'=>$_GET['type'],'show'=>1));
			echo json_encode($data);die;
		}
	}
        ...
        if ($pic != '') {
            ...
            if ($_POST['usertype'] == 3) {
                ...
                if ($img == 'logo') {
                    ...
                    $ltinfo =   $lietouM->getInfo(array('uid' => $this->uid), array('field' => '`photo_big`'));

                    if ($ltinfo['photo_big'] == '') {

                        $IntegralM->invtalCheck($this->uid, $this->usertype, "integral_avatar", "上传头像", 20);
                    }

                    $nid    =   $lietouM->upInfo(array('uid' => $this->uid), array('photo_big' => $pic));
                    if ($nid) {

                        $logM->addMemberLog($_POST['uid'], $_POST['usertype'], "上传猎头头像", 16, 1);
                    }
                }
            }
            if ($this->usertype == 4) {
                ...
                if ($img == 'logo') {
                    ...
                    $train  =   $trainM->getInfo(array('uid' => $this->uid), array('field' => 'logo'));

                    if ($train['logo'] == '') {

                        $IntegralM->invtalCheck($this->uid, $this->usertype, "integral_avatar", "上传LOGO", 20);
                    }
                    $nid    =   $trainM->upInfo(array('uid' => $this->uid), array('logo' => $pic));
                    if ($nid) {

                        $logM->addMemberLog($_POST['uid'], $_POST['usertype'], "上传培训机构LOGO", 16, 1);
                    }
                }
            }
        }
    ...
    function get_school_option_action()
    {
        ...
        $rowArr     =   $schoolM->getSchoolAcademyList($where, array('field' => '`id`,`schoolname`'));
        $rows       =   $rowArr['list'];
    ...
    function couponBuy_action(){
	    ...
	    $return    =   $couponM -> couponBuy($_POST);
	...
	//计划任务关闭房间
	function closeSpview_action(){
		$this->MODEL('spview')->closeRoom();
	}
	//发送预约视频面试的通知
	function sendSpviewSub_action(){
	    $this->MODEL('spview')->sendSubmsg();
	}
	//发送预约直播宣讲会的通知
	function sendXjhliveYy_action(){
	    $this->MODEL('xjhlive')->sendYymsg();
	}
	// 企业手动邀请视频面试，发送微信通知
