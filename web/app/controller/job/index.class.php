...
class index_controller extends job_controller{
	...
	function comsearch(){
		...
		$key = 'wxtz_tips';
		if($this->uid){
            $key = 'wxtz_tips_' . $this->uid;
            $userinfoM		=	$this->MODEL('userinfo');
            $member	=	$userinfoM->getInfo(array('uid'=> $this->uid),array('field'=>'`usertype`,`wxid`,`unionid`'));
        ...
        $this -> yunset('wxtz_tips', !empty($_COOKIE[$key]) ? 1 : 0); // 是否显示微信关注提示
	...
	function recommend_action(){
		...
		if(!empty($_POST)){
			...
			$userinfoM					=	$this -> MODEL('userinfo');
			//查询用户的昵称（个人用户 resume表，企业用户company表，猎头用户，培训用户）
			$nickname					=	'';
     ...
     * 关闭职位列表，微信通知提示
     */
    function closeWxtzTips_action(){
        $time = time();
        $key = $this->uid ? 'wxtz_tips_' . $this->uid : 'wxtz_tips';
        $this->cookie->setCookie($key, 1,$time + strtotime(date('Y-m-d 23:59:59')) - $time);
    }
}
?>