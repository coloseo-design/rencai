...
class admin_subject_controller extends siteadmin_controller{
	...
	//时间区间
	public $timeSection	=	array('1'=>'今天','3'=>'最近三天','7'=>'最近七天','15'=>'最近半月','30'	=>'最近一个月');
	...
	public function index_action(){
		...
		if(!empty($publishStr)){
			...
			$urlarr['publish']					=	intval($_GET['publish']);
		}
		//推荐条件
		if(!empty($_GET['rec'])){
			if ($_GET['rec'] == 2){
		...
		if(!empty($_GET['isprice'])){
			...
			$urlarr['isprice']					=	intval($_GET['isprice']);
		}
	...
	public function status_action(){
		...
		$subList								=	$pxM -> getSubList(array('id' => array('in', $id)), array('field' => '`uid`, `name`'));
		if(empty($subList)){
		...
		$pxList									=	$pxM -> getList(array('uid' => array('in', pylode(',', $uidArr))), $pxField);
		$pxListIndex							=	array();
		...
		//修改课程字段
		$nid									=	$pxM->upSubInfo(array('id' => array('in', $id)), array('status' => $postData['status'], 'statusbody' => $postData['statusbody']));
		if(!empty($nid)){
	...
	public function add_action(){
		...
		$this -> siteadmin_tpl(array('admin_subject_add'));
	}
	/**
	 * 会员- 培训 - 培训课程管理
	 * 保存课程数据
