 ...
 */
class admin_lt_pic_controller extends siteadmin_controller{
	function set_search()
    {
        $search_list[]  = array(
            'param'     => 'status',
	...
	public function index_action(){
		...
		if(!empty($keywordStr)){
			if($typeStr == 2){
				$ltWhereData['uid']	=	array('like', $keywordStr);
			}elseif($typeStr == 3){
    ...
    $this -> siteadmin_tpl(array('admin_lt_pic'));
	...
	public function del_action(){
		...
		//修改相应图片字段
		$delid									=	$ltM -> upInfo(array('uid' => array('in', $id)), array('photo' => ''));
		if(!empty($delid)){
