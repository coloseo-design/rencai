...
class comnews_controller extends siteadmin_controller{
	...
	function index_action(){
		...
		$rows             =    $companyM     ->      getCompanyNewsList($where,array('utype'=>'admin','cache'=>'1')); 
	...
	function status_action(){
		...
		if($allid){	
			...
			$Cnews              	=   $CompanyM   -> getCompanyNewsList($where,array('field'=>'uid,title'));
			/* 消息前缀 */		
			$tagName  				=	'企业新闻';
			...
			$nid?$this->ACT_layer_msg("企业新闻审核(ID:".$_POST['pid'].")设置成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg("设置失败！",8,$_SERVER['HTTP_REFERER']);
	...
	function del_action(){
		...
		$return		=    $CompanyM -> delCompanyNews($delID,array('uid'=>$this->uid,'usertype'=>$this->usertype));
