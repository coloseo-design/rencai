...
class email_controller extends siteadmin_controller{
	...
	function index_action(){
		...
		$this->siteadmin_tpl(array('admin_tuiguang'));
	}
	...
	function msgtg_action(){
		...
		$this->siteadmin_tpl(array('admin_msgtg'));
	}
	...
	function getBirthday_action(){
	    ...
	    $upjobA		=	$jobM->getList(array('lastupdate'=>array('<',strtotime('-7 day')),'r_status'=>1,'groupby'=>' `uid`','orderby'=>'lastupdate,desc'),array('field'=>'`uid`'));
		$upjob		=	$upjobA['list'];
	    foreach ($upjob as $v){
	        $upuid[]=$v['uid'];
	    }
	    if ($_GET['type']=='email'){
	        ...
	        $num['todaydue_e']   =	$companyM->getCompanyNum(array('email_status'=>'1','linkmail'=>array('<>',''),'r_status'=>1,'uid'=>array('in',pylode(',',$todayuid))));
			
	        $num['sevendue_e']   =	$companyM->getCompanyNum(array('email_status'=>'1','linkmail'=>array('<>',''),'r_status'=>1,'uid'=>array('in',pylode(',',$sevenuid))));
			...
			$addjob_ewh['r_status']		=	1;
			
			$addjob_ewh['linkmail']		=	array('<>','');
			
	        $num['addjob_e']     =	$companyM->getCompanyNum($addjob_ewh);
			
	        $num['upjob_e']      =	$companyM->getCompanyNum(array('email_status'=>'1','linkmail'=>array('<>',''),'r_status'=>1,'uid'=>array('in',pylode(',',$upuid))));
	    }else{
	        ...
	        $num['todaydue_m']   =	$companyM->getCompanyNum(array('moblie_status'=>'1','linktel'=>array('<>',''),'r_status'=>1,'uid'=>array('in',pylode(',',$todayuid))));
			
	        $num['sevendue_m']   =	$companyM->getCompanyNum(array('moblie_status'=>'1','linktel'=>array('<>',''),'r_status'=>1,'uid'=>array('in',pylode(',',$sevenuid))));
			...
			$addjob_mwh['r_status']			=	1;
			
			$addjob_mwh['linktel']			=	array('<>','');
			
	        $num['addjob_m']     =	$companyM->getCompanyNum($addjob_mwh);
			
	        $num['upjob_m']      =	$companyM->getCompanyNum(array('moblie_status'=>'1','linktel'=>array('<>',''),'r_status'=>1,'uid'=>array('in',pylode(',',$upuid))));
	    }
	...
	function sendPromotion_action(){
		...
		if($emailtype=='1'){
	        ...
	        if($this->config['sy_email_vipmr']=='2'){
	            $arr['status']=0;
	            $arr['msg']='请先开启会员到期提醒！';
	        ...
	        if($type=='2'){
	            $comjobsA	=	$jobM->getList(array('lastupdate'=>array('<',strtotime('-7 day')),'r_status'=>1,'groupby'=>'uid','orderby'=>'lastupdate,desc'),array('field'=>'`uid`,`lastupdate`'));
	...
	function shjobmsg($info,$type,$sort){
	    ...
	    if($data['type']!=""){
	        if($type=='1'){
	            ...
	            if($info['name']==''){
                    $userarr	        =	$this->MODEL('userinfo')->getMemberInfo(array('uid'=>$info['uid']),array('field'=>'`username`'));
	...
	function tgresume_action(){
	    $this->siteadmin_tpl(array('admin_tgresume'));
	}
	...
	function getcom_action(){
	    ...
	    if ($com==1){ 
			...
			$rowA	=	$jobM->getList(array('lastupdate'=>array('>',strtotime('-7 day',$time)),'r_status'=>1),array('field'=>'`uid`'));
			$row	=	$rowA['list'];
	    }elseif ($com==3){  
	...
	function getsendcom($com,$sendnum,$type=1){
	    ...
	    if ($com==1){ 
	        ...
	        $scomA	=	$jobM->getList(array('lastupdate'=>array('>',strtotime('-7 day')),'r_status'=>1),array('field'=>"distinct `uid`"));
			$scom	=	$scomA['list'];
	    }elseif ($com==3){  
	...
	function tgjob_action(){
	    $this->siteadmin_tpl(array('admin_tgjob'));
	}
	...
	function getuser_action(){
	    ...
	    $user		=	(int)$_POST['user'];
	    if ($user==1){
	        $row	=	$resumeM->getSimpleList(array('lastupdate'=>array('>',strtotime('-7 day')),'status'=>array('<>','2'),'r_status'=>1,'defaults'=>'1'),array('field'=>'`uid`'));
	    }elseif ($user==2){
	...
	function getsenduser($user,$sendnum,$type=1){
		...
		$userinfoM	=	$this->MODEL('userinfo');
	    if ($user==1){
	        $suser	=	$resumeM->getSimpleList(array('lastupdate'=>array('>',strtotime('-7 day')),'status'=>array('<>','2'),'r_status'=>1,'job_classid'=>array('<>',''),'defaults'=>'1'),array('field'=>'`uid`'));
	    }elseif ($user==2){ 
	    ...
	    if($suser&&is_array($suser)){
	        ...
	        if ($type==1){
	            ...
	            $resume	=	$resumeM->getSimpleList($where,"uid,name,email");
	        }else{
				$where['telphone']		=	array('<>','');
				
				$where['moblie_status']	=	'1';
				
	            $resume	=	$resumeM->getSimpleList($where,"uid,name,telphone");
	        }
	    }
	    return $resume;
	}
