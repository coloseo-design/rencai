...
class user_member_controller extends siteadmin_controller{
	...
	function index_action(){
		...
		if($_GET['status']){
		    ...
		    $memberwhere['status']          =  $status == 4 ? 0 : $status;
        ...
        if($List && is_array($List)){
            ...
            $mwhere['uid']		=			array('in',pylode(',',$uids));//查询member表
            //查询member表
            $mRows				=			$UserinfoM->getList($mwhere,array('field'=>'`uid`,`username`,`reg_date`,`login_date`,`source`,`status`'));
            foreach($List as $k=>$v){
              
                foreach($mRows as $val){

                  if($v['uid']	==		$val['uid']){

                    $List[$k]['username']		=		$val['username'];

                    $List[$k]['reg_date']		=		$val['reg_date'];

                    $List[$k]['login_date']		=	$val['login_date'];

                    $List[$k]['source']		=		$val['source'];
	...
	function lock_action(){
	    ...
	    $return     =  $userinfoM -> lock(array('uid'=>intval($_POST['uid']),'usertype'=>1),array('post'=>$post));
	...
	function status_action(){
	    ...
	    $return     =  $userinfoM -> status(array('uid'=>intval($_POST['uid']),'usertype'=>1),array('post'=>$post));
	...
	function member_log_action(){
      ...
      $this -> siteadmin_tpl(array('admin_user_member_log'));
	}
	/**
	 * 会员-个人-个人用户列表: 会员日志删除
	 */
	function memberlogdel_action(){
	    ...
	    if ($_GET['del']){
	        ...
	        $where  =  array('id'=>array('in',$_POST['del']));
	    }
	    $logM    =  $this -> MODEL('log');
	...
	function writtenOffLog_action(){
       ...
       $this -> siteadmin_tpl(array('admin_user_written_off_log'));
	}
