...
class index_controller extends common{
    ...
    function show_action(){
		...
		$departmentList		=	$CompanyaccountM -> getList(array('comid'=>$uid),array('field'=>'`name`,`uid`'));
		...
		if ($this->usertype==1){
            ...
            if(intval($isMsg) > 0){
                $msgList	=	$msgM -> getList(array('job_uid' => $uid,'status'=>1, 'orderby' => 'datetime,desc', 'limit' => 5));
		...
		$xjh      	=   $schoolM -> getSchoolXjhList(array('uid'=>$uid, 'status'=>'1', 'limit'=>'3'));
		$this     ->  yunset('xjh',$xjh);
		$this->yunset('operatime', $times);
		$this->data=$data;

        $WhbM       =   $this->MODEL('whb');
        $maxNum     =   $jobM->getJobNum(array('state' => 1, 'status' => 0, 'r_status' => 1, 'uid' => $uid));
        $syComHb    =   $WhbM->getWhbList(array('type' => 2, 'isopen' => '1', 'num' => array('<=', $maxNum), 'orderby' => 'num'));
	...
	function departmentjob_action(){
		...
		$limit		=	intval($_POST['limit'])?intval($_POST['limit']):'5';
		$updown		=	intval($_POST['updown'])?intval($_POST['updown']):'1';
		...
		$return		=	$JobM -> getList($where,array('isurl'=>'yes'));
		$html	=	"";
		foreach($return['list'] as $v){
			if($_POST['style']=="default"){
				$html .="<div class='firm_post'>";
					$html .="<div class='com_details_com_otherjob_l'>";
						$html .="<div class='com_details_com_otherjob_name'>";
							$html .="<a href='".$v['joburl']."' target='_blank'>".$v['name']."</a>";
						$html .="</div>";
						$html .="<div class='com_details_com_otherjob_info'>";
							if($v['job_exp']){
								$html .=$v['job_exp']."经验";
							}
							if($v['job_edu'] && $v['job_exp']){
								$html .="<span class='com_details_line'>|</span>".$v['job_edu']."学历";
							}
						$html .="</div>";
					$html .="</div>";
					$html .="<div class='com_details_com_otherjob_c'>";
						$html .="<div class='com_details_com_otherjob_xz'>".$v['job_salary']."</div>";
						$html .="<div class='com_details_com_otherjob_city'>".$v['job_city_two']."</div>";
					$html .="</div>";
					$html .="<div class='com_details_com_otherjob_r'>";
						$html .="<div class='com_details_com_otherjob_time'>".date('Y-m-d',$v['lastupdate'])."</div>";
						$html .="<a href='".$v['joburl']."' target='_blank'  class='com_details_com_otherjob_sq'>申请</a></div>";
					$html .="</div>";
				$html .="</div>";
			}else{
				$html .="<div class='firm_post'>";
					$html .="<div class='fpc_name'>";
						$html .="<a href='".$v['joburl']."' target='_blank'>".$v['name']."</a>";
					$html .="</div>";
					$html .="<div class='firm_post_joblist'>";
						if($v['job_salary']){
							$html .="<span class='comshow_job_xz'><i>".$v['job_salary']."</i></span>";
						}
						if($v['job_city_two']){
							$html .="<span class='comshow_job_city'>".$v['job_city_two']."</span>";
						}
						if($v['job_exp']){
							$html .="<span class='comshow_job_jy'>".$v['job_exp']."经验</span>";
						}
						if($v['job_edu']){
							$html .="<span class='comshow_job_xl'>".$v['job_edu']."学历</span>";
						}
					$html .="</div>";
					if($v['lastupdate']){
						$html .="<div class='firm_post_jobtime'>更新时间：".date("Y-m-d",$v['lastupdate'])."</div>";
					}
					$html .="<a href='".$v['joburl']."' target='_blank' class='firm_post_jobbth'>立即查看</a>";
				$html .="</div>";
			}
		}
		if($num>5){
			$html .="<div class='pages' style='margin-top:20px'>
			<a href=\"javascript:void(0);\" onclick=\"departmentjobpage('$_POST[comuid]','$_POST[departmentName]','$_POST[style]','$compage','5','1');\">上一页</a>
			<a href=\"javascript:void(0);\" onclick=\"departmentjobpage('$_POST[comuid]','$_POST[departmentName]','$_POST[style]','$compage','5','2');\">下一页</a>
			</div>";
		}
		echo $html;die;
	}
	...
	function msg_action(){
	    ...
	    $where     =   array('cuid'=>$cuid, 'status'=>'1');

	    $msglist   =   $CompanyM -> getCompanyMsgList($where, array('field'=>'jobid'));
	    ...
	    if(is_array($msglist) && $msglist){
	        ...
	        $jobA      =   $JobM -> getList(array('id'=>array('in', pylode(',', $jobid)), 'limit'=>'3'), array('field'=>'id, name'));
	...
	function productshow_action(){
		...
		$pl_num   =   $CompanyM -> getCompanyMsgNum(array('cuid'=>$uid,'status'=>1));
