...
class transfer_model extends model
{
                ...
                if (!empty($memberInfo)) {
                    ...
                    if ($res) {
                        ...
                        if ($data['bdtype']) {
                            ...
                            $this->update_once('lt_info', $lxData, array('uid' => $uid));

                            $this->update_once('px_train', $companyData, array('uid' => $uid));
                        }
                        ...
                        if (!empty($oldcom)) {
                            ...
                            $oldlt      =   $this->select_once('lt_info', array('uid' => $uid), '`uid`');
                            if (!empty($oldlt)) {
                                $memberData['usertype'] = 3;
                            } else {
                                $oldpx  =   $this->select_once('px_train', array('uid' => $uid), '`uid`');
                                if (!empty($oldpx)) {
                                    $memberData['usertype'] = 4;
                                }
                            }
                        }
                        if (!empty($memberData)) {
    ...
    private function filterTableUid($uid, $olduid, $user_moblie = 2)
    {
        ...
        // 其他字段特殊的数据表
        $this->update_once('company_job_rewardlog', array('uid' => $uid), array('uid' => $olduid, 'utype' => 1));

        $this->update_once('chat_log', array('uid' => $uid, 'from' => $uid), array('from' => $olduid, 'fusertype' => 1));

        $this->update_once('blacklist', array('c_uid' => $uid), array('c_uid' => $olduid, 'usertype' => 1));

        $this->update_once('chat_log', array('to' => $uid), array('to' => $olduid, 'tusertype' => 1));
        ...
        $nid = $this->update_once('subscribe', array('uid' => $uid), array('uid' => $olduid, 'type' => 1));
    ...
    public function mergeData($data = array())
    {
        ...
        /**
         *  判断个人是否有其他身份：企业、猎头、培训
         */
        $existCom   =   $this->select_num('company', array('uid' => $uid));
        $existLt    =   $this->select_num('lt_info', array('uid' => $uid));
        $existPx    =   $this->select_num('pr_train', array('uid' => $uid));

        if ((int)$existCom > 0 || (int)$existLt > 0 || (int)$existPx > 0){
        ...
        if (!isset($return['msg'])) {
            ...
            $isExistLt                          =   $this->select_once('lt_info', array('uid' => $com_uid));
            $isExistPx                          =   $this->select_once('px_train', array('uid' => $com_uid));
            ...
            if ($mobile == 1) {
                ...
                if (isset($isExistLt) && !empty($isExistLt)){

                    $ltData['moblie']           =   $user['moblie'];
                    $ltData['moblie_status']    =   $user['moblie_status'];
                }
                if (isset($isExistPx) && !empty($isExistPx)){

                    $pxData['linktel']          =   $user['moblie'];
                    $pxData['moblie_status']    =   $user['moblie_status'];
                }
            }
            ...
            if ($email == 1) {
                ...
                if (isset($isExistLt) && !empty($isExistLt)) {
                    ...
                    $ltData['email']            =   $user['email'];
                    $ltData['email_status']     =   $user['email_status'];
                }

                if (isset($isExistPx) && !empty($isExistPx)) {
                    
                    $pxData['linkmail']         =   $user['email'];
                    $pxData['email_status']     =   $user['email_status'];
                }
            }
            ...
            if(isset($ltData)){
                ...
                $this->update_once('lt_info', $ltData, array('uid' => $com_uid));
            }

            if (isset($pxData)){

                $this->update_once('px_train', $pxData, array('uid' => $com_uid));
            }
