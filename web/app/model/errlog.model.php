...
class errlog_model extends model{
     ...
     * @param string $uid
     * @param string $type
     * 1-注册，2-添加简历，3-申请职位，4-发布职位，5:-刷新职位,6:-下载简历，7-邀请面试,8-发送邮件，9-发送短信，10-微信相关
            ...
            if ($nid) {
                ...
                switch ($member['usertype']) {
                    ...
                    case '2':
                        ...
                        break;
                    case '3':
                        $wxTempMsg .= '，猎头用户《' . $userInfo['realname'] . '》' . $content;
                        break;
                    case '4':
                        $wxTempMsg .= '，培训用户《' . $userInfo['name'] . '》' . $content;
                        break;
                    case '5':
                        $wxTempMsg .= '，供求用户《' . $userInfo['name'] . '》' . $content;
                        break;
                }

                require_once('admin.model.php');
                $adminM =   new admin_model($this->db, $this->def);
                $adminM->sendAdminMsg(array('first' => $wxTempMsg, 'type' => 5));
            }
        }
    }
        ...
        if (!empty($List)) {
            ...
            foreach ($List as $key => $value) {
                ...
                if ($value['type'] == 1) {
                    ...
                    $List[$key]['type_n'] = '微信小程序';
                } elseif ($value['type'] == 11) {

                    $List[$key]['type_n'] = '内容检测';
                }
            }
            return $List;
        }
