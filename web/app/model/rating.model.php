...
class rating_model extends model{
	...
	public function ratingInfo($id = 0, $uid = 0, $add = 0)
    {
        ...
        if($statis['rating_type'] == $row['type'] && $row['type'] == 1 && $acc== 1){    //  套餐会员累加：后台选择当前会员累加 && 同为套餐会员
	        ...
	        if(isVip($statis['vip_etime'])){
	            ...
	            $value['chat_num']          =   array('+', (int)$row['chat_num']);
	            $value['spview_num']       	=   array('+', (int)$row['spview_num']);
	        }else{
	            $value['job_num']           =   array('=', (int)$row['job_num']);
	            $value['breakjob_num']      =   array('=', (int)$row['breakjob_num']);
	            $value['down_resume']       =   array('=', (int)$row['resume']);
	    ...
	    if($row['coupon'] > 0 && $row['time_start'] < time() && $row['time_end'] > time()){
	        $coupon							=	$this -> select_once("coupon", array('id' => array('=', $row['coupon'])));
	        $data							=	array();
	        $data['uid']					=	$uid;
	        $data['number']					=	time();
	        $data['ctime']					=	time();
	        ...
	        $data['validity']				=	time() + $coupon['time']*86400;
	        $data['coupon_amount']			=	$coupon['amount'];
	        $data['coupon_scope']			=	$coupon['scope'];
	        $this -> insert_into('coupon_list', $data);
	    }
     ...
     * @return array
     */
	public function ltratingInfo($id = 0, $uid = 0){

        $id			=	intval($id) == 0 ? $this->config['lt_rating'] : intval($id);
        $uid		=	intval($uid);
        ...
        $ltRatingAdd=   explode(',', $this->config['lt_rating_add']);
        $acc        =   in_array($statis['rating'], $ltRatingAdd) ? 1 : 2;

		//获取猎头会员等级
		$row 		=	$this -> getInfo(array('id' => $id, 'category' => 2));
		...
		if($statis['rating_type'] == $row['type'] && $row['type'] == 1 && $acc== 1){        //  套餐会员累加：后台选择当前身份会员累加 && 同为套餐会员
			...
			if($statis['vip_etime'] && isVip($statis['vip_etime'])){
				...
				$value['chat_num']          =   $row['chat_num'];
			}else{
				$value['lt_job_num']		=	$statis['lt_job_num'] + $row['lt_job_num'];
				$value['lt_breakjob_num']	=	$statis['lt_breakjob_num'] + $row['lt_breakjob_num'];
				$value['lt_down_resume']	=	$statis['lt_down_resume'] + $row['lt_resume'];
				$value['chat_num']	        =	$statis['chat_num'] + $row['chat_num'];
			}
			/*累加数据End*/
			$value['vip_etime']				=	$time;
    ...
    public function getList($whereData, $data = array())
    {
        ...
        if ($data['coupon'] == 1) {
            foreach ($ListRating as $v) {
                $couponid[] =   $v['coupon'];
            }
            if (!empty($couponid)) {
                require_once('coupon.model.php');
                $couponM    =   new coupon_model($this->db, $this->def);
                $coupon     =   $couponM->getList(array('id' => array('in', pylode(",", $couponid))), array('field' => 'id,name'));
            }
            if (is_array($coupon)) {
                foreach ($ListRating as $k => $v) {
                    foreach ($coupon as $val) {
                        if ($v['coupon'] == $val['id']) {
                            $ListRating[$k]['couponname']   =   $val['name'];
                            $ListRating[$k]['coupon']       =   $val['name'];
                        }
                    }
                }
            }
        ...
        if ($data['utype'] == 'lietou') {
            foreach ($ListRating as $k => $v) {
                $rname = array();
                if ($v['lt_job_num'] > 0) {
                    $rname[] = '猎头发布职位数:' . $v['lt_job_num'] . '份';
                }
                if ($v['lt_breakjob_num'] > 0) {
                    $rname[] = '猎头刷新职位数:' . $v['lt_breakjob_num'] . '份';
                }
                if ($v['lt_resume'] > 0) {
                    $rname[] = '猎头下载简历数:' . $v['lt_resume'] . '份';
                }
                if ($this->config['sy_chat_open'] == 1 && $v['chat_num'] > 0) {
                    $rname[] = '猎头' . $this->config['sy_chat_name'] . '数:' . $v['chat_num'] . '份';
                }
                $ListRating[$k]['rname'] = @implode('+', $rname);
            }
        }
	...
	function upComSerDetail($postData = array()) {
	    ...
	    if (isset($postData)) {
	        ...
	        $sort              =   intval($postData['sort']);
	        $chat_num          =   intval($postData['chat_num']);
	        $spview_num        =   intval($postData['spview_num']);
	...
	function fetchRatingInfo($fetData=array())
	{
		...
		$data['chat_num']			=		$row['chat_num'];
		$data['spview_num']			=		$row['spview_num'];
		...
		if($row['coupon']>0 && $fetData['uid']){
			$coupon						=		$this->select_once("coupon",array('id'=>$row['coupon']));
			$val						=		array();
			$val['uid']					=		$fetData['uid'];
			$val['number']				=		time();
			$val['ctime']				=		time();
	...
	public function changeRatingInfo($id = 0, $uid = 0)
    {
        ...
        if($statis['rating_type'] == $row['type'] && $row['type'] == 1 && $acc== 1){    //  套餐会员累加：后台选择当前身份会员累加  && 同为套餐会员
	        ...
	        if(isVip($statis['vip_etime'])){
	            ...
	            $value['chat_num']          =   $statis['chat_num'] + (int)$row['chat_num'];  
	            $value['spview_num']     	=   $statis['spview_num'] + (int)$row['spview_num'];  
	        }else{
	            ...
	            $value['chat_num']          =  	(int)$row['chat_num'];    
	            $value['spview_num']       	=  	(int)$row['spview_num'];    
	        }                                   
	        /*累加数据End*/
	...
	public function changeltratingInfo($id = 0, $uid = 0){
		...
		if($statis['rating_type'] == $row['type'] && $row['type'] == 1 && $acc== 1){            //  套餐会员累加： 后台选择当前身份会员累加  && 同为套餐会员
			...
			$value['chat_num']		        =	$row['chat_num'];
			$value['vip_etime']				=	$time;			
 		}else if($statis['rating_type'] != $row['type'] || $acc==2){                                //  直接覆盖: 时间类型和套餐类型相互转换 || 后台未选择当前身份会员累加
