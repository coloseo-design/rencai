...
class cache_model extends model{
     ...
     * Summary of GetCache
     * @param mixed $CacheType 缓存的类型：user、com、job、city、lt、ltjob、lthy、hy、subject
        ...
        foreach($CacheTypeList as $v){
            switch($v){
				...
				case 'integralclass':
				    ...
				    break;
				case 'crm':
			        $ReturnCacheList=array_merge_recursive($ReturnCacheList,$this->crm_cache($Options));
			        break;
				case 'uptime':
	...
	private function crm_cache($Options=array('needreturn'=>false,'needassign'=>true)){
	    include PLUS_PATH.'crm.cache.php';
	    //更新时间
	    if($Options['needreturn']==true){
	        return array('crmdata'=>$crmdata,'crmclass_name'=>$crmclass_name);
	    }
	}
