...
class coupon_model extends model{
	...
	*/
	 ...
	 */
	function upPreCard($whereData=array(),$addData=array(),$data=array()){
		
		$nid = $this	->	update_once('prepaid_card',$addData,$whereData);
     ...
     * $data            自定义处理数组
     */
	function usePreCard($whereData=array(),$data=array()){
		require_once ('integral.model.php');
        
		$info	=	$this	->	select_once("prepaid_card",$whereData);
		if($whereData['card']==""){
			$return['msg']	=	"请输入卡号！";
			$return['errcode']	=	8;
		}elseif($whereData['password']==""){
			$return['msg']	=	"请输入密码！";
			$return['errcode']	=	8;
		}elseif(empty($info)){
			$return['msg']	=	"卡号或密码错误！";
			...
			$odata['did']			=	$data['userdid'];
			$odata['integral']		=	$integral;
			$odata['type']			=	'2';
			$nid	=	$this	->	insert_into("company_order",$odata);
			if($nid){
				$this		->	upPreCard(array('id'=>$info['id']),array('uid'=>$data['uid'],'username'=>$data['username'],'utime'=>time()));
	...
	function getPreCardInfo($whereData, $data = array()) {
	    
	    $field      =   $data['field'] ? $data['field']  : '*';
	    
	    $info       =   $this -> select_once('prepaid_card', $whereData, $field);
        ...
        if($data['ratingid'] && $data['coupon_id']){
            ...
            if(empty($ratinginfo)){
                ...
                if($coupon['coupon_amount'] >= $price && $coupon['coupon_scope'] <= $price){
                    ...
                    $this -> addMemberLog($uid, $usertype, '全额使用优惠券，'.$ratinginfo['name'].'购买成功');
	                ...
	                if($coupon['coupon_amount'] >= $price && $coupon['coupon_scope'] <= $price){
	                    ...
	                    if ($status) {
	                        ...
	                        if ($jobnum == 1) {
	                            ...
	                            $this -> addMemberLog($data['uid'], $data['usertype'], '批量刷新职位(ID:'.$sxjobid.')', 1, 4);
	                        }
	                ...
	                if($coupon['coupon_amount'] >= $price && $coupon['coupon_scope'] <= $price){
	                    ...
	                    if ($status) {
	                        ...
	                        if ($partnum == 1) {
	                            ...
	                            $this->addMemberLog($uid, $data['usertype'], '刷新兼职(ID:'.$parts[0]['id'].')《'.$parts[0]['name'].'》', 9, 4);
	                        } else {
	                            
	                            $this->addMemberLog($uid, $data['usertype'], '批量刷新兼职(ID:'.$sxpartid.')', 9, 4);
	                        }
	 ...
	 */
	private function buyRefreshLtJob($data)
	{
	            ...
	            if (empty($ltjobs)) {
	                ...
	                if($coupon['coupon_amount'] >= $price && $coupon['coupon_scope'] <= $price){
	                    ...
	                    if ($status) {
	                        ...
	                        $this -> upCouponList(array('id' => $couponid, 'uid' => $uid), array('status' => 2, 'xf_time'=>time()));
	                        
	                        $integral->company_invtal($uid, $usertype, $price, false, '使用优惠券，购买猎头职位刷新', true, 2, 'yhq', 12, '', $couponid);
	                        
	                        if ($ltjobnum == 1) {
	                            
	                            $this->addMemberLog($data['uid'], $usertype, '刷新猎头职位(ID:'.$ltjobs[0]['id'].')《' . $ltjobs[0]['job_name'] . '》', 10, 4);
	                        } else {
	                            $this->addMemberLog($data['uid'], $usertype, '批量刷新猎头职位(ID:'.$sxltjobid.')', 10, 4);
	                        }
