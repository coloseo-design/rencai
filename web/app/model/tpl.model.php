...
class tpl_model extends model{
	 ...
	 * $addData		自定义
	 */
	function upTplmobliepic($addData=array(),$whereData=array()){
		if(!empty($addData)){
			
			$nid	=	$this	->	update_once('tplmobliepic',$addData,$whereData);
	 ...
	 * $whereData 	查询条件
	 */
	function delTplmobliepic($delId){
		...
		if(!empty($delId))
		{
			...
			$nid		=	$this	->	delete_all('tplmobliepic',array('id'=>array('in',$delId)),'');
		}
		return	$nid;
	}
	/*
