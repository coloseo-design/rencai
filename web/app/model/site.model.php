		...
		if(!empty($data)){
			...
			if(is_array($rids)&&$rids) {
				...
				$Table	=	array(
					...
					'company_msg','invoice_record','look_job','member_statis',
					
					'resume','resume_expect','user_entrust','userid_job','gq_info', 'gq_task', 'gq_browse'
				);
			}
			if(is_array($comids)&&$comids) {
				...
				$Table	=	array(
					...
					'company_product','invoice_record','partjob','hotjob','user_change','company_account','gq_info', 'gq_task', 'gq_browse'
				);
				$this -> updDid(array('userid_msg'), array('fid' => array('in', $uid)), $didData);
				
				$this -> updDid(array('ad_order'),array('comid'=>array('in', $uid)),$didData);
			}
			if(is_array($ltids)&&$ltids) {
				
				$Table	=	array(
					'member', 'company_cert', 'company_order',
					
					'lt_info', 'lt_job', 'lt_statis', 'invoice_record','gq_info', 'gq_task', 'gq_browse'
				);
			}
			if(is_array($pxids)&&$pxids) {
				
				$Table	=	array(
					'member', 'company_cert', 'company_order',
					
					'px_train','px_subject','px_train_news','px_teacher','gq_info', 'gq_task', 'gq_browse'
				);
				
				$this -> updDid(array("px_zixun"), array('s_uid' => array('in', $uid)), $didData);
			}
