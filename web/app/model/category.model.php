...
class category_model extends model{
	 ...
	 * $field		自定义查询字段  
	 */
	function getSchoolClass($whereData=array(),$field='*'){
		$one	=	array();
		$one	=	$this	->	select_once('schoolclass',$whereData,$field);
		return	$one;
	}
	/*
	 * 获取校园类别列表
	 ...
	 * $field		自定义查询字段 
	 */
	function getSchoolClassList($whereData=array(),$field='*'){
		$list	=	array();
		$list	=	$this	->	select_all('schoolclass',$whereData,$field);
		return	$list;
	}
	/*
	 * 添加或更新校园类别
	 ...
	 * $whereData 	更新分类查询条件 
	 */
	function addSchoolClass($addData=array(),$whereData = array()){
		...
		if(!empty($whereData)){
			...
			$this	->	update_once('schoolclass',$addData,$whereData);
			$this 	->  cache_action('school_cache','school');
			$showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
			$this	->	adminLog("校园分类".$showid.$type."修改成功");
		}else{
			$name				=	array();
			foreach ($addData['name'] as $val){
				if($val){
			...
			//检查提交的类别名称是否有同名
			$class			=	$this	->	getSchoolClassList($where);
			$valueData			=	array();
			...
			if(empty($class)){//没有同名类则正常添加
				...
				$ucid 			=	$this	->	DB_insert_multi('schoolclass',$valueData);
				$this 			->  cache_action('school_cache','school');
				$return['msg']	=	$ucid ? 2 : 3;
				$this 			->  adminLog("校园分类添加成功");
			}else{//有同名类，给出提示
				$return['msg']	=	1;
			}
		}
	 ...
	 * $whereData 	删除分类查询条件 
	 */
	function delSchoolClass($whereData=array(),$data=array()){

		$id					=	array();
		$sclass				=	$this	->	getSchoolClassList($whereData,'id');
		foreach ($sclass as $key => $value) {
			$id[]			=	$value['id'];
		}
		$ids				=	$this	->	sonclass('schoolclass',$id);//获取当前项及其所有子类的id集合
		if(count($ids)){
			$result	=	$this -> delete_all('schoolclass',array('id'=>array('in',pylode(',',$ids))),'');
		}
		$this 				->  cache_action('school_cache','school');
		$return['errcode']	=	$result ? '9' :'8';
		$return['layertype']=	$data['type']=='all' ? 1 : 0;
		$return['msg']		=	$result ? '校园分类删除成功！' : '删除失败！';
		return	$return;
	}
	/////////////////////////////////////////////////////////职位类别管理//////////////////////////////////////////////////////////////
	/*
	 ...
	 * $field		自定义查询字段  
	 */
	function getLtClass($whereData=array(),$field='*'){
		$one	=	array();
		$one	=	$this	->	select_once('ltclass',$whereData,$field);
		return	$one;
	}
	/*
	 * 获取猎头会员类别列表
	 ...
	 * $field		自定义查询字段 
	 */
	function getLtClassList($whereData=array(),$field='*'){
		$list	=	array();
		$list	=	$this	->	select_all('ltclass',$whereData,$field);
		return	$list;
	}
	/*
	 * 添加或更新猎头会员类别
	 ...
	 * $whereData 	更新分类查询条件 
	 */
	function addLtClass($addData=array(),$whereData = array()){
		...
		if(!empty($whereData)){
			...
			$this	->	update_once('ltclass',$addData,$whereData);
			$this 	->  cache_action('lt_cache','lt');
			$showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
			$this	->	adminLog("猎头会员分类".$showid.$type."修改成功");
		}else{
			$name				=	array();
			foreach ($addData['name'] as $val){
				if($val){
			...
			//检查提交的类别名称是否有同名
			$class			=	$this	->	getLtClassList($where);
			$valueData			=	array();
			...
			if(empty($class)){//没有同名类则正常添加
				...
				$ucid 			=	$this	->	DB_insert_multi('ltclass',$valueData);
				$this 			->  cache_action('lt_cache','lt');
				$return['msg']	=	$ucid ? 2 : 3;
				$this 			->  adminLog("猎头会员分类添加成功");
			}else{//有同名类，给出提示
				$return['msg']	=	1;
			}
		}
	 ...
	 * $whereData 	删除分类查询条件 
	 */
	function delLtClass($whereData=array(),$data=array()){

		$id					=	array();
		$sclass				=	$this	->	getLtClassList($whereData,'id');
		foreach ($sclass as $key => $value) {
			$id[]			=	$value['id'];
		}
		$ids				=	$this	->	sonclass('ltclass',$id);//获取当前项及其所有子类的id集合
		if(count($ids)){
			 
			$result	=	$this -> delete_all('ltclass', array('id'=>array('in',pylode(',',$ids))), '');
		}
		$this 				->  cache_action('lt_cache','lt');
		$return['errcode']	=	$result ? '9' :'8';
		$return['layertype']=	$data['type']=='all' ? 1 : 0;
		$return['msg']		=	$result ? '猎头会员分类删除成功！' : '删除失败！';
		return	$return;
	}
	///////////////////////////////////////////////////////猎头行业分类//////////////////////////////////////////////////////////////
	/*
	 ...
	 * $field		自定义查询字段  
	 */
	function getLthyClass($whereData=array(),$field='*'){
		$one	=	array();
		$one	=	$this	->	select_once('lthy_class',$whereData,$field);
		return	$one;
	}
	/*
	 * 获取猎头行业类别列表
	 ...
	 * $field		自定义查询字段 
	 */
	function getLthyClassList($whereData=array(),$field='*'){
		$list	=	array();
		$list	=	$this	->	select_all('lthy_class',$whereData,$field);
		return	$list;
	}
	/*
	 * 添加或更新猎头行业类别
	 ...
	 * $whereData 	更新分类查询条件 
	 */
	function addLthyClass($addData=array(),$whereData = array()){
		...
		if(!empty($whereData)){
			...
			$this	->	update_once('lthy_class',$addData,$whereData);
			$this 	->  cache_action('lthy_cache','lthy');
			$showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
			$this	->	adminLog("猎头行业分类".$showid.$type."修改成功");
		}else{
			$name				=	array();
			foreach ($addData['name'] as $val){
				if($val){
			...
			//检查提交的类别名称是否有同名
			$class			=	$this	->	getLthyClassList($where);
			$valueData			=	array();
			...
			if(empty($class)){//没有同名类则正常添加
				...
				$ucid 			=	$this	->	DB_insert_multi('lthy_class',$valueData);
				$this 			->  cache_action('lthy_cache','lthy');
				$return['msg']	=	$ucid ? 2 : 3;
				$this 			->  adminLog("猎头行业分类添加成功");
			}else{//有同名类，给出提示
				$return['msg']	=	1;
			}
		}
	 ...
	 * $whereData 	删除分类查询条件 
	 */
	function delLthyClass($whereData=array(),$data=array()){

		$id					=	array();
		$sclass				=	$this	->	getLthyClassList($whereData,'id');
		foreach ($sclass as $key => $value) {
			$id[]			=	$value['id'];
		}
		$ids				=	$this	->	sonclass('lthy_class',$id);//获取当前项及其所有子类的id集合
		if(count($ids)){
			 
			$result	=	$this -> delete_all('lthy_class', array('id'=>array('in',pylode(',',$ids))), '');
		}
		$this 				->  cache_action('lthy_cache','lthy');
		$return['errcode']	=	$result ? '9' :'8';
		$return['layertype']=	$data['type']=='all' ? 1 : 0;
		$return['msg']		=	$result ? '猎头行业分类删除成功！' : '删除失败！';
		return	$return;
	}
	///////////////////////////////////////////////////////猎头职位分类//////////////////////////////////////////////////////////////
	/*
	 ...
	 * $field		自定义查询字段  
	 */
	function getLtjobClass($whereData=array(),$field='*'){
		$one	=	array();
		$one	=	$this	->	select_once('ltjob_class',$whereData,$field);
		return	$one;
	}
	/*
	 * 获取猎头行业类别列表
	 ...
	 * $field		自定义查询字段 
	 */
	function getLtjobClassList($whereData=array(),$field='*'){
		$list	=	array();
		$list	=	$this	->	select_all('ltjob_class',$whereData,$field);
		return	$list;
	}
	/*
	 * 添加或更新猎头职位类别
	 ...
	 * $whereData 	更新分类查询条件 
	 */
	function addLtjobClass($addData=array(),$whereData = array()){
		...
		if(!empty($whereData)){
			...
			$this	->	update_once('ltjob_class',$addData,$whereData);
			$this 	->  cache_action('ltjob_cache','ltjob');
			$showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
			$this	->	adminLog("猎头职位分类".$showid.$type."修改成功");
		}else{
			$name				=	array();
			foreach ($addData['name'] as $val){
				if($val){
			...
			//检查提交的类别名称是否有同名
			$class			=	$this	->	getLtjobClassList($where);
			$valueData			=	array();
			...
			if(empty($class)){//没有同名类则正常添加
				...
				$ucid 			=	$this	->	DB_insert_multi('ltjob_class',$valueData);
				$this 			->  cache_action('ltjob_cache','ltjob');
				$return['msg']	=	$ucid ? 2 : 3;
				$this 			->  adminLog("猎头职位分类添加成功");
			}else{//有同名类，给出提示
				$return['msg']	=	1;
			}
		}
	 ...
	 * $whereData 	删除分类查询条件 
	 */
	function delLtjobClass($whereData=array(),$data=array()){

		$id					=	array();
		$sclass				=	$this	->	getLtjobClassList($whereData,'id');
		foreach ($sclass as $key => $value) {
			$id[]			=	$value['id'];
		}
		$ids				=	$this	->	sonclass('ltjob_class',$id);//获取当前项及其所有子类的id集合
		if(count($ids)){
			 
			$result	=	$this -> delete_all('ltjob_class', array('id'=>array('in',pylode(',',$ids))), '');
		}
		$this -> cache_action('ltjob_cache','ltjob');
		$return['errcode']	=	$result ? '9' :'8';
		$return['layertype']=	$data['type']=='all' ? 1 : 0;
		$return['msg']		=	$result ? '猎头职位分类删除成功！' : '删除失败！';
		return	$return;
	}
	///////////////////////////////////////////////////////举报原因//////////////////////////////////////////////////////////////////
	/*
	 ...
	 * $field		自定义查询字段  
	 */
	function getSubjectClass($whereData=array(),$field='*'){
		$one	=	array();
		$one	=	$this	->	select_once('px_subject_class',$whereData,$field);
		return	$one;
	}
	/*
	 * 获取课程类别列表
	 ...
	 * $field		自定义查询字段 
	 */
	function getSubjectClassList($whereData=array(),$field='*'){
		$list	=	array();
		$list	=	$this	->	select_all('px_subject_class',$whereData,$field);
		return	$list;
	}
	/*
	 * 添加或更新课程类别
	 ...
	 * $whereData 	更新分类查询条件 
	 */
	function addSubjectClass($addData=array(),$whereData = array()){
		...
		if(!empty($whereData)){
			...
			$this	->	update_once('px_subject_class',$addData,$whereData);
			$this 	->  cache_action('subject_cache','subject');
			$showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
			$this	->	adminLog("课程类别".$showid.$type."修改成功");
		}else{
			$name				=	array();
			foreach ($addData['name'] as $val){
				if($val){
			...
			//检查提交的类别名称是否有同名
			$class			=	$this	->	getSubjectClassList($where);
			$valueData			=	array();
			...
			if(empty($class)){//没有同名类则正常添加
				...
				$ucid 			=	$this	->	DB_insert_multi('px_subject_class',$valueData);
				$this 			->  cache_action('subject_cache','subject');
				$return['msg']	=	$ucid ? 2 : 3;
				$this 			->  adminLog("课程类别添加成功");
			}else{//有同名类，给出提示
				$return['msg']	=	1;
			}
		}
	 ...
	 * $whereData 	删除分类查询条件 
	 */
	function delSubjectClass($whereData=array(),$data=array()){

		$id					=	array();
		$sclass				=	$this	->	getSubjectClassList($whereData,'id');
		foreach ($sclass as $key => $value) {
			$id[]			=	$value['id'];
		}
		$ids				=	$this	->	sonclass('px_subject_class',$id);//获取当前项及其所有子类的id集合
		if(count($ids)){
			 
			$result	=	$this -> delete_all('px_subject_class', array('id'=>array('in',pylode(',',$ids))), '');
		}
		$this -> cache_action('subject_cache','subject');
		$return['errcode']	=	$result ? '9' :'8';
		$return['layertype']=	$data['type']=='all' ? 1 : 0;
		$return['msg']		=	$result ? '课程类别删除成功！' : '删除失败！';
		return	$return;
	}
	///////////////////////////////////////////////////////开课类型//////////////////////////////////////////////////////////////////
	/*
	 ...
	 * $field		自定义查询字段 
	 */
	function getSubjectTypeClassList($whereData=array(),$field='*'){
		$list	=	array();
		$list	=	$this	->	select_all('px_subject_type',$whereData,$field);
		return	$list;
	}
	/*
	 * 添加开课类型
	 * $addData 	提交分类数据 'name':名称为数组形式封装
	 */
	function addSubjectTypeClass($addData=array()){
		$valueData	=	array();
		
		$whereData['name']	=	array('in',"'".@implode("','", $addData['name'])."'");
		$industry			=	$this	->	getSubjectTypeClassList($whereData);

		if(empty($industry)){
		    foreach ($addData['name'] as $key=>$val){
		        $valueData[$key]['name']=$val;
		    }
		    $result	=	$this	->	DB_insert_multi('px_subject_type',$valueData);
		    $this	->	cache_action('subject_type_cache','subject_type');
		    $return	=	$result	?	2	:	3;
		    $this	->	adminLog("开课类型添加成功！");
		}else{
			$return=1;
		}
		return	$return;
	 ...
	 * $whereData 	更新查询
	 */
	function upSubjectTypeClass($addData=array(),$whereData=array()){
		...
		$this	->	update_once('px_subject_type',$addData,$whereData);
		$showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
		$this	->	cache_action('subject_type_cache','subject_type');
		$this	->	adminLog("开课类型".$showid.$type."修改成功");
	}
	/*
	 * 删除开课类型
	 * $data 		自定义数组数据
	 ...
	 * $whereData 	删除分类查询条件 
	 */
	function delSubjectTypeClass($whereData=array(),$data=array()){
		if($data['type']=='one'){//单个删除
			$limit			=	'limit 1';
		}
		if($data['type']=='all'){//多个删除
		...
		$result	=	$this -> delete_all('px_subject_type', $whereData, $limit);
		$this ->  cache_action('subject_type_cache','subject_type');
		$return['errcode']	=	$result ? '9' :'8';
		$return['layertype']=	$data['type']=='all' ? 1 : 0;
		$return['msg']		=	$result ? '开课类型删除成功！' : '删除失败！';
		return	$return;
	}
	 ...
	 * $field		自定义查询字段
	 */
	function getCrmClass($whereData=array(),$field='*'){
	    $one	=	array();
	    $one	=	$this	->	select_once('crm_client_class',$whereData,$field);
	    return	$one;
	}
	/*
	 * 获取个人会员分类列表
	 ...
	 * $whereData 	更新分类查询条件
	 */
	function addCrmClass($addData=array(),$whereData = array()){
	    ...
	    if(!empty($whereData)){
	        ...
	        $this	->	update_once('crm_client_class',$addData,$whereData);
	        $this 	->  cache_action('crm_cache','crm');
	        $showid	=	$whereData['id']	?	"(ID:".$whereData['id'].")"	:	'';
	        $this	->	adminLog("CRM客户分类".$showid.$type."修改成功");
	    }else{
	        $name				=	array();
	        foreach ($addData['name'] as $val){
	            if($val){
	        ...
	        //检查提交的类别名称是否有同名
	        $userclass			=	$this	->	getCrmClassList($where);
	        $valueData			=	array();
	        ...
	        if(empty($userclass)){//没有同名类则正常添加
	            ...
	            $ucid 			=	$this	->	DB_insert_multi('crm_client_class',$valueData);
	            $this 			->  cache_action('crm_cache','crm');
	            $return['msg']	=	$ucid ? 2 : 3;
	            $this 			->  adminLog("CRM客户分类添加成功");
	        }else{//有同名类，给出提示
	            $return['msg']	=	1;
	        }
	    }
	 ...
	 * $whereData 	删除项查询条件(只需要当前选择删除的项，不需要包含子类查询)
	 */
	function delCrmClass($whereData=array(),$data=array()){
	    $id					=	array();
	    $cclass				=	$this	->	getCrmClassList($whereData,'id');
	    foreach ($cclass as $key => $value) {
	        $id[]			=	$value['id'];
	    }
	    $ids				=	$this	->	sonclass('crm_client_class',$id);//获取当前项及其所有子类的id集合
	    if(count($ids)){
			 
	        $result	=	$this -> delete_all('crm_client_class',array('id'=>array('in',pylode(',',$ids))),'');
	    }
	    $this ->  cache_action('crm_cache','crm');
	    $return['errcode']	=	$result ? '9' :'8';
	    $return['layertype']=	$data['type']=='all' ? 1 : 0;
	    $return['msg']		=	$result ? 'CRM分类删除成功！' : '删除失败！';
	    return	$return;
	}
