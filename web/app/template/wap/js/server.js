...
$("ul#rating_select").on("click", "li", function() {
    ...
    if(integral && pro && only_price_arr.indexOf(server)==-1){	// 积分模式
        ...
        $(".order_integral_div").css('display', 'none');
        $(".order_coupon_div").css('display', 'none');
...
function couponPicker(){
    ...
    $.post('index.php?c=getCouponList',{price:server_price, cid: cid},function(data){
        data = eval('(' + data + ')');
    ...
    server_vue.$data.coupon = couponData;
    server_vue.$data.couponIndex = couponIndex;
    server_vue.$data.couponShow = true;
}
...
function chCoupon() {
    ...
    var coupon_name = $("#couponValue").html();

    var server_price = $("#server_price").val();
    var integral_dk = $("#integral_dk").val();
    var server = $("#server").val();

    if (only_price_arr.indexOf(server) != -1) {
        // 后台设置购买会员强制金额消费，购买会员服务不能使用积分抵扣
        integral_dk = 0;
        $(".integral_dk_div").css('display', 'none');
    } else {
        $(".integral_dk_div").css('display', 'flex');
    ...
    if (coupon_price && coupon_id) {
        ...
        if (parseFloat(coupon_price) >= parseFloat(server_price)) {	//	优惠券金额大于单品金额

            $(".order_coupon_div").css('display', 'flex');
            $(".order_price_div").css('display', 'none');
            $(".order_integral_div").css('display', 'none');
            $(".integral_dk_div").css('display', 'none');
            $(".pay_style_div").css('display', 'none');
            $("#order_coupon").html(server_price);
            ...
            var price = accSub(parseFloat(server_price), parseFloat(coupon_price));		//	优惠券使用后金额

            var integral_need = accMul(price, integral_pro);
            ...
            if (parseInt(integral_need) <= parseInt(integral_dk)) {
                ...
                if (integral_dk) {
                    integral_need = accSub(parseInt(integral_need), parseInt(integral_dk));
                }

                $("#order_price").html(accDiv(integral_need, integral_pro));
            }
        }

        $('#couponValue').html(coupon_name);
        ...
        if (parseInt(integral_dk) > 0) {
            ...
            var price = accDiv(parseInt(integral_dk), integral_pro);

            if (parseFloat(price) < parseFloat(server_price)) {

                $("#order_price").html(accSub(parseFloat(server_price), parseFloat(price)));
                ...
                $("#order_integral").html(parseInt(integral_dk));
...
function checkIntegralDK(integral, pro){
    ...
    var coupon_price	=	$("#coupon_price").val();		//	使用优惠券金额
    var coupon_id		=	$("#coupon_id").val();			//	使用优惠券ID
    ...
    if(coupon_price && coupon_id){
        ...
        server_price	=	accSub(parseFloat(server_price),parseFloat(coupon_price));	// 	下单金额
    }
...
function paycheck(type){
    ...
    if(type == 'wxpay'){
        ...
        $(".dredge_body_wx .dredge_body_wx_icon img").attr('src', imgSurl);
    }else if(type == 'alipay'){
...
function orderBuy(){
    ...
    if (price == 0 && single_id != ''){
        ...
        if (server == 'issuejob' || server == 'invite' || server == 'createson' || server == 'spview') {
            ...
            pData.chatid = single_id;
        } else if (server == 'zphnet') {
...
function integralBuy(){
    ...
    if(server == 'issuejob'){
        ...
        var rurl = wapurl + 'index.php?c=chat&type=1&id=' + single_id;
        pData.chatid = single_id;
    }else if(server == 'zphnet'){

        var rurl = wapurl + 'index.php?c=zphnet&a=show&id=' + single_id;
        pData.zid = single_id;
    }else if(server == 'spview'){

        var rurl  	= 	wapurl + 'member/index.php?c=spview';
...
function couponBuy(){
    ...
    if(server == 'issuejob'){
        ...
        var rurl = wapurl + 'index.php?c=spview';
        pData.zid = single_id;
    }
    var ajaxUrl		= 	'index.php?c=couponBuy';
    $.post(ajaxUrl, pData, function(data){
        hideLoading();
        var data = eval('(' + data + ')');
