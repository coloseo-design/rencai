...
function selCoupon(){
	...
	var server_price	=	$("#server_price").val();		//	所选服务金额 
 	...
 	$.post('index.php?c=getCouponList',{price:server_price, cid: cid},function(data){
 		...
 		$("#coupon_li").html(data.h1);
 		
 		var couponLayer = layer.open({
 			
 			type	: 1,
 			content	: $("#coupon_div").html(),
 			anim	: 'up',
...
/*layui 监听 ul li click 事件*/
$(document).on("click",'ul#coupon_li li',function(){
	
	$(this).siblings('li').removeClass("yhq_xz_list_cur"); //	删除兄弟li的class属性
 	$(this).addClass("yhq_xz_list_cur");	
		...
		if(event.detail.isActive) {
			...
			if(document.getElementById('integral_dk_single_div')) { // 兑换积分部分隐藏
				...
				if(coupon_price){
					...
					$("#order_price").html(accSub(parseFloat(server_price), parseFloat(coupon_price)));
				}else{
...
function checkIntegralDK(integral, pro){
	...
	var coupon_id		=	$("#coupon_id").val();			//	使用优惠券ID
 	...
 	if(coupon_price && coupon_id){
 		...
 		var server_price	=	accSub(parseFloat(server_price),parseFloat(coupon_price));	// 	下单金额
 	}else{
