...
function Check_jobadd(){
	...
	if(min==''||min=='0'){
		layer.msg("请输入职位年薪",2,8);return false;
	}
...
function blurinfotype(id){
	...
	}else if(id=="cityid"&&$("#cityid").val()==""){
		$("#by_cityid").attr("class","m_name_byy");
		$("#by_cityid").html($("#cityid").attr("date"));
	}else if(id=="provinceid"&&$("#provinceid").val()==""){
		$("#by_provinceid").attr("class","m_name_byy");
		$("#by_provinceid").html($("#provinceid").attr("date"));
	}else{
		$("#by_"+id).attr("class","m_name_gh");
		$("#by_"+id).html('');
	}
