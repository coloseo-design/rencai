...
function check_user(id,img){
	...
	if($("#xieyi"+id).attr("checked")!='checked'){  
		...
		var param = {username:username, password:password, email:email, moblie:moblie, moblie_code:moblie_code, authcode:authcode, codeid:id, verify_token:verify_token};
		if (sy_reg_type == 2){

			param.reg_name = reg_name;
			param.reg_link = reg_link;
			param.reg_type = reg_type;
			param.reg_bind = reg_bind;
		}
